 ...
 ?>
  <div class="bss-slides demo1" tabindex="1" autofocus="autofocus">
  	@foreach($getSlide as $item) 
            <figure>
		      <img src="{{Content::urlImage($item->picture, '/photos/share/')}}" style="background-image: url({{Content::urlImage($item->picture, '/photos/share/')}});" width="100%" /><figcaption style="float: left; padding:11px 66px;"><h2>{{$item->name}}</h2></figcaption> 
            </figure>
@endforeach
